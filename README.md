...
<details> <summary><h3>Лабораторная работа 3</h3></summary>

**Цель**:
Научиться связывать логику расчетов (математику) с графическим интерфейсом WinForms.

**Задание**:
Создать WinForms приложение, в котором игрок "тренирует" персонажа, чтобы победить главного босса.

---

### **Структура интерфейса**

#### **Первое окно (Главное меню/Тренировка):**
* **Статистика**: Метки (`Label`) для отображения характеристик: "Сила", "Ловкость" и "Золото".
* **Действия**:
    * Кнопка **"Тренироваться"**: дает +1 к Силе, стоит 0 золота.
    * Кнопка **"Работать"**: дает +10 золота.
* **Магазин**: `ComboBox` или `ListBox` с предметами (например: "Гантели", "Витамины"). При покупке характеристики персонажа увеличиваются.
* **Переход**: Кнопка **"В бой!"**, которая открывает второе окно (Арену).

#### **Второе окно (Арена):**
* **Визуализация**: Используйте `PictureBox` для отображения героя и босса (допустимо использовать простые цветные фигуры).
* **Логика боя**:
    * Здоровье босса = 100 единиц.
    * Урон героя = `(Сила * 2)`.
    * Шанс критического удара зависит от показателя "Ловкость".
    * Реализуйте кнопку **"Ударить"**. Если здоровье босса падает до 0 — победа (`MessageBox`).

---

### **Технические требования**

1.  **Валидация**: Нельзя купить предмет в магазине, если не хватает золота (выводить `MessageBox`).
2.  **События**: Использовать `TextChanged` или `SelectedIndexChanged` для динамического обновления цены предмета при его выборе в списке.
3.  **Таймер (Дополнительно)**:
    * Добавьте `Timer`, который каждую секунду отнимает 1 единицу "Энергии".
    * Если энергия равна 0 — кнопки тренировки блокируются (`Enabled = false`).
...
* **Передача данных между формами**:
    Создайте конструктор во второй форме, который принимает необходимые параметры:
    ```csharp
    BattleForm battle = new BattleForm(heroStrength, heroAgility);
    battle.Show();
    ```

* **Расчет критического удара**:
    Используйте класс `Random` для определения вероятности:
    ```csharp
    Random rnd = new Random();
    int chance = rnd.Next(1, 101); // Генерация числа от 1 до 100
    ```

</details>

<details> <summary><h3>Лабораторная работа 4</h3></summary>

Цель:
Работа с динамическими коллекциями объектов, реализация поиска по списку и изменение свойств объектов в реальном времени.

Задание:
Разработать приложение для управления остатками товаров на складе. У каждого товара должно быть название, количество и категория.
...
* Добавление: Ввод названия, количества и выбор категории товара через `ComboBox`.
* Сортировка/Фильтрация: Кнопка «Показать только дефицит» (выводит в список только те товары, у которых количество < 3 шт).
* Статистика: Отдельное поле (например, `Label`), которое динамически считает общую стоимость склада или общее количество коробок.
Новые технические фишки (Усложнение)

    Инвентаризация (Изменение внутри списка):
    * Справа от списка товаров (`ListBox`) должны быть расположены кнопки «+» и «-».
    * При нажатии на них количество выбранного в `ListBox` товара должно увеличиваться или уменьшаться прямо в объекте (внутри коллекции `List<StockItem>`).
    * *Сложность*: Необходимо найти объект в списке по индексу, изменить его поле и корректно обновить строку в интерфейсе.

    Живой поиск:
    * Добавить `TextBox` для текстового поиска.
    * При вводе текста список должен автоматически фильтроваться (скрывать элементы, которые не подходят под название).
    * *Подсказка*: Для реализации используется метод `.Contains()`.

    Визуальная индикация (События):
    * Если количество выбранного товара становится меньше 5 единиц, `Label` с остатком должен окрашиваться в красный цвет и выводить предупреждение: *_«СРОЧНО ТРЕБУЕТСЯ ЗАКУПКА!»_*.
